...
The **OptiEmptyLatent** node calculates and generates an empty latent tensor with an optimal resolution for a given aspect ratio and megapixel (MP) target, tailored for different model types in ComfyUI. It supports both automatic (optimized) and manual (exact resolution) modes. Only preset model configurations are available.
...
- **Latent**
  - The generated empty latent tensor (shape: `(batch_size, channels, height//vae_scale, width//vae_scale)`).
    - Channels and VAE scale factor are determined by the selected model preset and are not user-editable.
...
- **Block Size**
  - The block size used for the calculation (preset by model).
...
- The node automatically clamps aspect ratios to the recommended range for the selected model. If your aspect ratio is outside the recommended range, a warning will be shown in the Details output and the value will be clamped for best results.
...
- Block size and VAE scale factor are determined by the selected model preset and cannot be changed by the user.
